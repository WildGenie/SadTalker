...
def save_video_with_watermark(video, audio, save_path, watermark=False):
    ...
    cmd = f'ffmpeg -y -i "{video}" -i "{audio}" -vcodec copy "{temp_file}"'
    os.system(cmd)
    ...
    if watermark is False:
        ...
        cmd = f'ffmpeg -y -hide_banner -i "{temp_file}" -i "{watarmark_path}" -filter_complex "[1]scale=100:-1[wm];[0][wm]overlay=(main_w-overlay_w)-10:10" "{save_path}"'
        os.system(cmd)
        os.remove(temp_file)